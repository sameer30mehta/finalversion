...
export const runValuation = async (inputs) => {
  ...
  const coarseBucket = assignCoarseBucket(lat, lon, cityTier);

  // ─── LAYER 7: Micro-Market Bucket Assignment ───
  const microMarket = assignMicroMarketBucket(lat, lon);
  ...
  if (enrich.legalStatus) confidence += 0.05;
  ...
  const legalScore = enrich.legalStatus === "clear" ? 10 : (enrich.legalStatus === "lease" ? 7 : 2);
  rpi += legalScore * (10 / 10);
  rpi += 10 * (8 / 10);
  rpi += (enrich.occupancy === "vacant" ? 4 : 8) * (5 / 10);
  rpi += 7 * (3 / 10);
  rpi += (enrich.legalStatus === "clear" ? 10 : 5) * (5 / 10);

  rpi = Math.round(rpi);

  let distressDiscount = cityTier === 1 ? 0.15 : (cityTier === 2 ? 0.20 : 0.25);
  if (age > 20) distressDiscount += 0.05;
  if (enrich.legalStatus && enrich.legalStatus !== "clear") distressDiscount += 0.08;
  ...
  if (enrich.occupancy === "vacant" || enrich.legalStatus !== "clear" || age > 25) {
  ...
  const anomalyResults = runAnomalyPipeline(
    { area: inputs.propertyDetails.area, config, type, age },
    coarseBucket,
    microMarket,
    hyperlocalContext,
    hasImages,
    enrich.legalStatus === 'clear'
  );
  ...
  if (anomalyResults.decision.decision === 'ACCEPT_CONFIDENCE_PENALTY') {
  ...
  return {
    ...
    confidenceBreakdown: {
      base: 0.60,
      legal: enrich.legalStatus === 'clear' ? 0.05 : 0,
